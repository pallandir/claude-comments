---
name: redline
description: Watch for UI comments left through the Redline browser extension and implement them directly in the source code. Use when the user says "/redline", "watch for comments", "pick up the UI comments", "apply the frontend feedback", or pastes a session id to start watching. Binds to the session id, runs a self-contained watch loop, and delegates each comment batch to a sub-agent for implementation. Reads from the redline MCP server (bind_session, wait_for_update, list_comments, get_comment, resolve_comment, defer_comment, list_deferred) with a fallback to .claude/design-comments.md.
---
...
**There is no "Send" button and nothing for the user to sync by hand.** A comment
reaches the store the moment the user clicks Save in the composer. Never tell the
user to "send" or "sync" a comment.
...
**Step 2 — List.**
Call `list_comments` with status `open`. Empty → loop back to step 1.

**Step 3 — Fetch.**
Call `get_comment(id)` for each open comment to get its source hint, screenshot
path, operator (XPath), operation, and the `plan-first` line.

**Step 4 — Plan-first partition (no sub-agent).**
For any comment where the rendered output contains `plan-first: true`, call
...
**Step 5 — Spawn one sub-agent for the remaining batch.**
...
**Step 6 — Resolve or defer based on the sub-agent's return.**
...
- `implemented` → call `resolve_comment(id, "resolved")`.
- `needs-plan` → call `defer_comment(id, "<reason from sub-agent>", "claude")`.
...
- The user set `plan-first: true` (shown in `get_comment` output as `plan-first: true`).
...
- The sub-agent does not call MCP tools. Only you, the watcher, call `bind_session`,
  `wait_for_update`, `resolve_comment`, `defer_comment`, and `list_deferred`.
